...
class RoomScene {
    ...
    updateSnapshot(snapshot: StageSnapshot): void {
...
export class PhaserRoomStage {
    ...
    mount(initialState: GameState): void {
        if (!window.Phaser || this.game) {
            return;
        }

        (window as any).__season1StageSnapshot = this.makeSnapshot(initialState);
            ...
            scene: [
                {
                    create(this: any) {
                        ...
                        roomScene.create();
                        roomScene.updateSnapshot((window as any).__season1StageSnapshot);
                        (window as any).__season1RoomScene = roomScene;
                    },
                },
            ],
        });
    ...
    render(state: GameState): void {
        (window as any).__season1StageSnapshot = this.makeSnapshot(state);
        const roomScene = (window as any).__season1RoomScene as RoomScene | undefined;
        roomScene?.updateSnapshot(this.makeSnapshot(state));
    }

    private makeSnapshot(state: GameState): StageSnapshot {
        return {
