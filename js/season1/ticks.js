...
function applyModeBase(state, deltas) {
...
function applySlots(state, slots, deltas) {
    slots.forEach((slot) => {
        if (slot === 'rest_slot') {
            deltas.energy += 12;
            deltas.stress -= 7;
...
function applyRoomAndStress(state, deltas) {
...
function applyCharacters(state, slots, deltas) {
...
function applyProjectProgress(state, deltas) {
...
function unlockCharacters(state) {
...
function increaseReadiness(state) {
    const selected = new Set(state.routine.socialPriority);
    Object.keys(state.relationships).forEach((characterId) => {
        const relationship = state.relationships[characterId];
        if (!relationship.unlocked) {
            return;
        }
        relationship.readiness = clamp(relationship.readiness + (selected.has(characterId) ? 10 : 4) - (state.routine.lifeMode === 'side_hustle' ? 2 : 0), 0, 100);
    });
}
function resolveAlerts(state) {
    const alerts = [];
    if (state.resources.money < 180) {
...
function resolvePositiveStates(state, deltas) {
    const positives = [];
...
function buildCycleTitle(state) {
...
function buildCycleSummary(state, deltas) {
    const parts = [];
