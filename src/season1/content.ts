import type { ActivityMode, CharacterId, ProjectGenre, ProjectPlatform, ProjectSize } from './types.js';
...
export const GENRE_DATA: Record<ProjectGenre, { label: string; icon: string; codingW: number; designW: number }> = {
...
export const PLATFORM_DATA: Record<ProjectPlatform, { label: string; icon: string; earningsM: number; fansM: number }> = {
...
export const SIZE_DATA: Record<ProjectSize, { label: string; days: number; baseEarnings: number; baseFans: number }> = {
...
export const ACTIVITY_DATA: Record<ActivityMode, { label: string; icon: string; hint: string }> = {
...
export const CHARACTER_DATA: Record<CharacterId, { name: string; role: string; bonus: string; unlockDesc: string }> = {
...
const GENRE_NOUNS: Record<ProjectGenre, string[]> = {
...
export function generateProjectName(genre: ProjectGenre): string {
    const prefix = PREFIXES[Math.floor(Math.random() * PREFIXES.length)];
    const nouns = GENRE_NOUNS[genre];
    return `${prefix} ${nouns[Math.floor(Math.random() * nouns.length)]}`;
}

export function getReviewQuote(rating: number): string {
    if (rating >= 9.0) return pick(['Это шедевр. Один человек, одна комната, и вот этот результат.', 'Вы серьёзно сделали это в одиночку?', 'Лучший инди-релиз года. Точка.']);
    if (rating >= 7.5) return pick(['Крепко. Чувствуется вложенная работа.', 'Не без шероховатостей, но с душой.', 'Приятный сюрприз от маленькой студии.', 'Видно: разработчик знает что делает.']);
    if (rating >= 5.5) return pick(['Ну, работает. Чего-то не хватает, но сыграть можно.', 'Типичный инди. Ни плохо, ни хорошо.', '«Играбельно» — лучший комплимент.', 'Очередной клон, но честный.']);
    if (rating >= 3.5) return pick(['Разработчик явно старался. Просто получилось не очень.', 'Потенциал виден. Реализация подкачала.', 'Это существует. Уже кое-что.', 'Хм. Следующий будет лучше, наверное.']);
