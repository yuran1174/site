...
import type { ActivityMode, BootPayload, CharacterId, ProjectGenre, ProjectPlatform, ProjectSize } from './types.js';
...
const ui = new Season1UI({
    onActivityChange(mode: ActivityMode) {
        state.activity = mode;
        persistAndRender();
    },
    onSocialTargetChange(id: CharacterId) {
    ...
    onStartProject(genre: ProjectGenre, platform: ProjectPlatform, size: ProjectSize) {
        state.activeProject = createProject(genre, platform, size);
        if (state.activity !== 'work') state.activity = 'work';
        persistAndRender();
    },
    onDismissRelease() {
...
// Auto-tick
window.setInterval(() => {
    if (state.pendingRelease) return; // pause while release screen open
    resolveDays(state, 1);
    persistAndRender();
}, TICK_MS);
